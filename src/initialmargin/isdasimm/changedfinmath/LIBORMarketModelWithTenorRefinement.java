...
public class LIBORMarketModelWithTenorRefinement extends AbstractModel implements TermStructureModelInterface {
	...
	private void doCalculateNumeraire(int timeIndex) throws CalculationException{ // Unadjusted numeraires
		RandomVariableInterface numeraire = null;
		if(timeIndex == 0) {
	...
	public RandomVariableInterface getForwardBondLibor(double T, double t) throws CalculationException{
		...
		for(int i = initialIndex; i<finalIndex; i++){
			...
			RandomVariableInterface factor = getLIBOR(getTimeDiscretization().getTimeIndexNearestLessOrEqual(t), getLiborPeriod(i),getLiborPeriod(i+1)).mult(liborPeriodLength).add(1.0).invert();
	...
	public double getLiborPeriod(int timeIndex) {
		if(timeIndex >= liborPeriodDiscretizations[0].getNumberOfTimes() || timeIndex < 0) {
			throw new ArrayIndexOutOfBoundsException("Index for LIBOR period discretization out of bounds: " + timeIndex + ".");
		}
		return liborPeriodDiscretizations[0].getTime(timeIndex);
	}
	
	public int getLiborPeriodIndex(double time) {
		return liborPeriodDiscretizations[0].getTimeIndex(time);
	}
